...
See the 'Access from Native Apps' section for details. Note that you'll need to
write your own app for this. We don't provide a sample.
...
1. Ruby 2.2.2, Rails 4.2.2, Angular ~1.3
1. Multitenancy!! (see the multi-tenant branch)
1. Puma web server
1. Postgres support
1. Complete user management, with authentication and role-based authorization
1. Complete user upload management and viewing, via an attachment library
1. User authentication via Facebook, Google and other providers
1. Front-end asset management using Bower
1. Angular-based routing
1. User confirmation before navigating away, such as from unsaved form fields
   (works for Angular route changes, *and* when navigating to external URLs)
1. Angular-based flash messages (can include HTML and even Angular directives)
...
1. Full featured admin console (i.e. an admin app) for administrators
...
1. Run the following command in the application's root directory:
...
1. Create two databases named as follows, or edit /config/database.yml to suit
   ...
   * rails-app-starter-kit_test (for testing)
   ...
   overridden anyway, but its still good to change them, for completeness.

1. Run the following command:

    $ rake db:migrate

   This will set up the required tables in the database. If a rake version
   mismatch throws up an error, run the command (and all future rake commands)
   in the bundler's context as follows: 'bundle exec rake db:migrate'.

1. Change the session key in /config/initializers/session_store.rb to suit your
   application.

1. To set up outgoing email, set the following environment variables:
   ...
   The APPLICATION_HOST variable is required for development and production. The
   others are only required in production. You need only set them in development
   if you want to send real emails in development.

1. The Puma web server supports concurrent request handling, by allowing
   multiple worker processes per machine and multiple threads per worker.

   If you wish to customize worker and/or thread concurrency, set any or all of
   ...
   IMPORTANT: Although we believe the app to be thread-safe, there is no way to
   guarantee it! If you encounter any race conditions, starvation or deadlock,
   just set MAX_THREADS to 1 (for now).

1. By default, the app is configured to support authentication via 3rd party
   providers. If you don't need this functionality, do the following:
   * Comment out the ':omniauthable...' line in /app/models/user.rb
   * Comment out the 'controllers:...' line with the 'devise_for' directive in
     /config/routes.rb

   To allow authentication via Facebook, create a Facebook app if you haven't
   already, and set the following environment variables:

    FACEBOOK_APP_ID=<Your Facebook App ID>
    FACEBOOK_APP_SECRET=<Your Facebook App Secret>

   To allow authentication via Google, create some Google OAuth2 credentials if
   ...
   To disable authentication via either one of Facebook or Google, modify the
   ':omniauthable...' line in /app/models/user.rb appropriately (it is
   self-explanatory).
...
1. If you wish to allow API-based access for native apps, set the following
   ...
   From your native app, you must then send this token with each request, thru
   the 'X-App-Access-Token' header.

1. If you wish to use the attachment library for handling and viewing uploads,
   set the following environment variables. This requires some work, but is
   worth it!
    ...
    AWS_CF_DISTRIBUTION=<The CloudFront distribution linked to the bucket,
      starting with the protocol (http:// or https://)>
    JW_PLAYER_ACCOUNT_TOKEN=<JWPlayer account token>

   And only for production, set this too:

    DRAGONFLY_CDN=<The CDN used to serve Dragonfly (gem) processed images such
      as thumbnails etc.>

   The origin of the Dragonfly CDN must be set to the domain name of your app.

   We recommend you provision an AWS IAM user with limited permissions, so that
   all unrelated AWS data remains secure. These permissions must of course
   include uploading and deleting objects from the bucket.

   Also ensure that the bucket allows CORS, including the DELETE method:
   http://docs.aws.amazon.com/AmazonS3/latest/dev/cors.html

   Our opinionated view is that S3 objects must always be accessed through
   CloudFront, rather than directly. To set up CloudFront, see:
   http://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/PrivateContent.html

   Also create a CloudFront Key Pair, to be used for generating signed URLS for
   protected access to your S3 objects. This Key Pair will be available for
   download as a .pem file.

   Set AWS_CF_PRIVATE_KEY to the entire contents of the .pem file as one long
   string, manually inserting a '\\n' at the end of each line. If setting via
   the command line, you should enclose the contents within double quotes.

   For playing back video attachments, we provide JWPlayer (an HTML5 and Flash
   based video player) integration out of the box. If you haven't already, you
   will need to sign up for an account at http://www.jwplayer.com/sign-up, so
   you can get an account token.

   If you want to use another player, you'll need to provide your own Angular
   directive for it (easy enough). In this case, you need not set the
   JW_PLAYER_ACCOUNT_TOKEN environment variable.

1. Run the Rails console, and create an admin user as follows:

    > u = User.new email: 'some@email.com', password: 'somepassword'
    > u.add_role :admin
    > u.skip_confirmation!
    > u.save!

   For the moment you need to create the admin user manually. However, we will
   be providing some seed data soon, which will include an admin user.

== Running the App (RubyMine)

This section only applies if you are using RubyMine.

Since we maintain a Procfile for declaratively running the app, we need to set
up RubyMine to use this Procfile, by creating a custom run/debug configuration.
The default RubyMine configuration does not pick up the Procfile.

First, install the 'foreman' gem:

 $ gem install foreman

Note that if you have Heroku toolbelt installed, it is likely that foreman was
installed with it. Even if this is the case, some older versions of foreman
throw an error when loading the application's YAML config files. In this case,
get the latest version of foreman by doing the following:

 $ gem uninstall foreman
 $ gem install foreman

Then, set up RubyMine as follows:

1. Click on Run > Edit Configurations
1. Click on the '+' button, then select 'Gem Command'
1. Enter the following: Name: Foreman, Gem name = foreman,
   Executable name = foreman, Arguments = start

As tested on RubyMine 7.1.3, this configuration works for both running and
debugging. Be sure to set all environment variables on this configuration, NOT
the default ones. Also set the environment variables on the 'Rails console: xxx'
configuration, for the Rails console to work correctly!

Running/Debugging via this configuration will start up both the web and worker
(for background jobs) processes.

== App Structure

The application structure is similar to a standard Rails app, with certain
additions, a mostly comprehensive sampling of which is shown below:
...
we use Bower, which is a very widely used such tool. We use the 'bower-rails'
...
By default, Rails logging is too verbose, making it difficult to find specific
log entries. We use the lograge[https://github.com/roidrage/lograge] gem to
fix this. See also this relevant file (search for Lograge within):
...
We also provide authentication with Facebook and Google out of the box, as well
as easy integration with other providers such as Twitter and more.

See the 'Setting Up' section for how to enable Facebook/Google authentication.

==== Authentication via Other 3rd Party Providers
...
extent of route access. See the bottom of the Authentication section above.

=== Attachment Library

Most applications need some way of:
1. Storing and viewing files uploaded by users
2. Associating these uploads with models (for example, an attachment to a post,
   a user profile pic etc.

We provide a full featured, flexible solution right out of the box!
This includes a server-side DSL for declaratively configuring models to accept
attachments, and corresponding Angular directives for uploading, attaching and
viewing them.

The only catch is that we use FineUploader[http://fineuploader.com/] to carry
out the actual uploads. If you're not developing an open-source application, a
license must be purchased. The same goes for JWPlayer, an HTML5 and Flash based
video player we use for playing back video attachments. Neither is very costly.

Attachments belong to users. Users only ever have access to their own
attachments. Each attachment can be attached to multiple objects - there is a
many to many relationship between attachments and what they are attached to.

See the following for more information:

* Client-side code:
  * /app/assets/javascripts/shared/angular/modules/attachment_library/
  * /app/assets/javascripts/client/angular/controllers/app_ctrl.js
  * /app/assets/javascripts/shared/angular/directives/jw_player.js

* Server-side code:
  * /app/models/attachment.rb
  * /app/models/attachment_join.rb
  * /app/controllers/attachments_controller.rb
  * /app/controllers/attachment_joins_controller.rb
  * /app/controllers/fine_uploader_controller.rb
  * /app/policies/* (namesake policies for above controllers)
  * /config/routes.rb
...
form validation via the 'fbErrors' directive.

We also provide 'fb-field', a directive for succinctly creating form fields.
This integrates automatically with 'fb-errors' to display errors, while at the
same time making the form HTML much less verbose and repetitive.

For more information, see files in
/app/assets/javascripts/shared/angular/modules/form_builder/directives.
...
The DataTables jQuery plugin forms an integral part of the admin app, giving
admins the ability to filter, sort and operate on data conveniently.
...
We've provided tight integration with Angular as well as Rails, and added many
great custom features to the standard plugin:
1. datatable Angular directive
1. DataTableAdapter Ruby class for doing the grunt work of parsing data table
1. Declarative single-row operations (such as edit/delete)
1. Multi-row selection and custom bulk operations (such as bulk delete)
1. The ability to render Angular directives within data table cells
1. Advanced multi-condition data filtering when combined with the query-builder
   directive - itself a highly flexible utility

See the following relevant files:
* /app/assets/javascripts/shared/angular/directives/data_table.js
* /app/models/data_table_adapter.rb
* /app/assets/javascripts/admin/angular/controllers/posts_ctrl.js
* /app/assets/javascripts/admin/angular/controllers/users_ctrl.js
* /app/assets/javascripts/templates/admin/controllers/posts/index.html
* /app/assets/javascripts/templates/admin/controllers/users/index.html
* /app/assets/javascripts/shared/angular/modules/query_builder/
...
This is a very common task, for which we use the excellent
Moment.js[http://momentjs.com] library. We generally prefer the format 'lll'
(see their home page for information) or similar, since it is locale-based, and
not hard-coded.
