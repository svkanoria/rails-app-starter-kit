...
1. Some basic CSS structure and styles
...
If you with to change the Ruby version, remember to also do so in the Gemfile
(near the top).
...
== Basic CSS Structure and Styling

We provide some very basic CSS structure, and some styles using Yahoo's 'Pure'
CSS framework. All styles can be found in the app/assets/stylesheets directory.

The Pure framework is included simply via its CDN, so there is no dependency on
Bower and the asset pipeline.
