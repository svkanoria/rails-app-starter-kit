...
Please treat any data (posts, users etc.) as transient. These may not be
...
throw an error when loading the applications' YAML config files. In this case,
do the following, to tget the latest version of foreman:
...
1. If you wish to user the attachment library, set the following environment
   variables:
   ...
   The origin of the Dragonfly CDN must be the domain name of your app.
   ...
   directive for it. In this case, you need not set the JW_PLAYER_ACCOUNT_TOKEN
   environment variable.
...
1. Storing files uploaded by users
...
video player we use for playing back video attachments.
