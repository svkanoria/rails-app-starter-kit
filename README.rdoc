...
Most applications need some way of storing attachments and other files uploaded
by users, and of associating uploads with models. For example, an attachment to
a post, a user profile pic etc.
...
Attachments belong to users. Users only have access to their own attachments at
all times. Each attachment can be attached to multiple objects.
