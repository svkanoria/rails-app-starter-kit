...
Since we are using the Puma web server with a config file (/config/puma.rb), we
need a custom run/debug configuration for the app. The default RubyMine ones do
not pick up the Puma config file.

Do so as follows:

1. Click on Run > Edit Configurations
1. Click on the '+' button, then select 'Gem Command'
1. Enter the following: Name: Puma, Gem name = puma, Executable name = puma,
   Arguments = -C config/puma.rb
...
To run a worker process for background jobs, run the following command:
