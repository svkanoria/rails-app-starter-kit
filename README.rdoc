...
1. Rails ~4.2, Angular ~1.3 (and we recommend Ruby >=2.1.2)
1. Puma web server
1. Postgres support
...
As tested on RubyMine 7.1, this configuration works for both running and
...
1. The Puma web server supports concurrent request handling, by allowing
   ...
   The defaults are 3 and 1 respectively.

   IMPORTANT: Don't fiddle with MAX_THREADS for now, as we have not yet tested
   the app's thread safety. We will be working on this soon.
